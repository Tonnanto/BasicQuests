...
public class TestCommand extends BasicQuestsCommand {
    ...
    private void testQuestGeneration(@NotNull BasicQuestsPlugin plugin, @NotNull CommandSender sender, QuestPlayer player, int amount) {
    ...
    public void checkQuests(List<Quest> quests) {
        ...
        for (Quest quest : quests) {
            ...
            if (questTypeConfig.getOptions() != null && !questTypeConfig.getOptions().isEmpty()) {
                ...
                Optional<GenerationOption> generationOption = questTypeConfig.getOptions().stream().filter(option -> option.getName().equals(materialName))
                        .findFirst();
                assert generationOption.isPresent();

                int minAmount = (generationOption.get().getMin() == 0) ? questTypeConfig.getDefaultMin() : generationOption.get().getMin();
                int maxAmount = (generationOption.get().getMax() == 0) ? questTypeConfig.getDefaultMax() : generationOption.get().getMax();
                int step = (generationOption.get().getStep() == 0) ? questTypeConfig.getDefaultStep() : generationOption.get().getStep();
    ...
    public void increaseMaterialForQuestType(String materialString, QuestType questType) {
        ...
        Optional<GenerationOption> materialOption = materialCounterMap.keySet().stream()
                .filter(generationOption -> generationOption.getName().equals(materialString)).findFirst();
    ...
    private void validateAndDisplayMaterialCountsForQuestType() {
        ...
        for (Map.Entry<QuestType, Map<GenerationOption, Integer>> materialMapEntry : optionCounterForQuestType.entrySet()) {
            ...
            for (Map.Entry<GenerationOption, Integer> materialEntry : materialMapEntry.getValue().entrySet()) {
                ...
                String materialCountString = String.format("%-20s count: %-5s        %5.2f %%    (%-5.2f%% expected)", materialEntry.getKey().getName(),
                        materialEntry.getValue(), actualWeight * 100, materialEntry.getKey().getWeight() / totalOptionWeight * 100);
