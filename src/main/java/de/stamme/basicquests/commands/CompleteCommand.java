...
public class CompleteCommand extends BasicQuestsCommand {
  ...
  public void evaluate(
    ...
    QuestPlayer questPlayer =
        BasicQuestsPlugin.getPlugin()
            .getQuestPlayers()
            .get(player.getUniqueId()); // ;.getQuestPlayer(player);
    ...
    if (params.size() != 2) {
      BasicQuestsPlugin.sendRawMessage(
          ...
          MessageFormat.format(
              MessagesConfig.getMessage("generic.usage"), "completequest [player] [index]"));
      return;
    }
  ...
  /**
   * Called when a CommandSender tries to complete a QuestPlayers quest by its index sender and
   * target can be the same player! Sender -> /quests complete <target> [questIndex]
   *
   * @param sender the CommandSender who executed the command
   ...
   */
  private void onCompleteQuestByIndex(
      CommandSender sender,
      QuestPlayer target,
      int questIndex,
      boolean clicked,
      @Nullable String clickedQuestID) {
    ...
    if (target.getQuests().size() > questIndex) {
      ...
      if (clicked && (questID == null || !questID.equals(clickedQuestID))) {
        BasicQuestsPlugin.sendMessage(
            sender, MessagesConfig.getMessage("commands.complete.already-completed"));
        return;
      }
    }
  ...
  /**
   * Called when a CommandSender tries to complete a players quest sender -> /quests complete
   * <target> [questIndex] sender -> /quests complete <target>
   *
   * @param sender the CommandSender who executed the command
   ...
   * @param questIndex the index of the quest that should be completed
   */
  private void onCompleteQuestForOther(
      CommandSender sender,
      String targetName,
      boolean clicked,
      @Nullable String clickedQuestID,
      @Nullable Integer questIndex) {
    // check permission
    if (!sender.hasPermission(getPermission() + ".others")) {
  ...
  /**
   * Shows a list of all possible quests to skip for the given player. Prompts the sender to select
   * a quest by clicking it in the chat. A ClickEvent will be fired if a quest is clicked. This
   * event will execute another /quests complete command with the index.
   *
   * @param player the player to be prompted
   * @param target the players whose quest should be skipped
   * @param targetNameArgument the target's name to put in the new command. Null if selector and
   *     target are the same player.
   */
  public void promptCompleteSelection(
      Player player, QuestPlayer target, @Nullable String targetNameArgument) {
    BasicQuestsPlugin.sendRawMessage(
        player,
        player == target.getPlayer()
            ? MessagesConfig.getMessage("commands.complete.header")
            : MessageFormat.format(
                MessagesConfig.getMessage("commands.complete.header-other"), target.getName()));
    ...
    BasicQuestsPlugin.sendRawMessage(
        ...
        player == target.getPlayer()
            ...
            : MessageFormat.format(
                MessagesConfig.getMessage("commands.complete.footer-other"), target.getName()));
  }
}
