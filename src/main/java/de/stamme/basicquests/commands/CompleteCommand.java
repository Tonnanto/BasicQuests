...
public class CompleteCommand extends BasicQuestsCommand {
     ...
     * @param questIndex the index of the quest that should be completed
     */
    private void onCompleteQuestForOther(CommandSender sender, String targetName, boolean clicked, @Nullable String clickedQuestID,
    ...
    /**
     * Shows a list of all possible quests to skip for the given player. Prompts the sender to
     * select a quest by clicking it in the chat. A ClickEvent will be fired if a quest is clicked.
    ...
    public void promptCompleteSelection(Player player, QuestPlayer target, @Nullable String targetNameArgument) {
        BasicQuestsPlugin.sendRawMessage(player,
        ...
        for (int i = 0; i < target.getQuests().size(); i++) {
            ...
            BasicQuestsPlugin.sendRawMessage(player, MessageFormat.format(MessagesConfig.getMessage("commands.complete.format"),
                    quest.getInfo(i + 1, false, false), targetNameArgument != null ? targetNameArgument + " " : "", i + 1, quest.getId()));
        }
