...
public class SkipCommand extends BasicQuestsCommand {
      ...
      if (skipsLeft <= 0 && !sender.hasPermission("basicquests.admin.skip.unlimited")) {
        questPlayer.sendMessage(
            MessageFormat.format(
                MessagesConfig.getMessage("commands.skip.none"), StringFormatter.timeToMidnight()));
        return;
      }
    ...
    if (argsLen == 1) {
      ...
      try {
        ...
        if (skipsLeft <= 0 && !sender.hasPermission("basicquests.admin.skip.unlimited")) {
          questPlayer.sendMessage(
              MessageFormat.format(
                  MessagesConfig.getMessage("commands.skip.none"),
  ...
  private void onConsoleSkipQuest(CommandSender sender, List<String> params) {
    if (params.size() != 2) {
      BasicQuestsPlugin.sendRawMessage(
          sender,
          MessageFormat.format(
              MessagesConfig.getMessage("generic.usage"), "skipquest [player] [index]"));
      return;
    }
  ...
  /**
   * Called when a CommandSender tries to skip a QuestPlayers quest by its index sender and target
   * can be the same player! Sender -> /quests skip <target> [questIndex]
   *
   * @param sender the CommandSender who executed the command
   ...
   */
  private void onSkipQuestByIndex(
    ...
    if (target.getQuests().size() > questIndex) {
      ...
      if (clicked && (questID == null || !questID.equals(clickedQuestID))) {
        BasicQuestsPlugin.sendMessage(
            sender, MessagesConfig.getMessage("commands.skip.already-skipped"));
        return;
      }
    }
  ...
  /**
   * Called when a CommandSender tries to skip a players quest sender -> /quests skip <target>
   * [questIndex] sender -> /quests skip <target>
   *
   * @param sender the CommandSender who executed the command
   ...
   * @param questIndex the index of the quest that should be skipped
   */
  private void onSkipQuestForOther(
      CommandSender sender,
      String targetName,
      boolean clicked,
      @Nullable String clickedQuestID,
   ...
   */
  public void promptSkipSelection(
    ...
    BasicQuestsPlugin.sendRawMessage(
        ...
        player == target.getPlayer()
            ...
            : MessageFormat.format(
                MessagesConfig.getMessage("commands.skip.footer-other"), target.getName()));
