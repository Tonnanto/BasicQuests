...
public class SkipCommand extends BasicQuestsCommand {
    ...
    public void promptSkipSelection(Player player, QuestPlayer target, @Nullable String targetNameArgument) {
        BasicQuestsPlugin.sendRawMessage(player,
                player == target.getPlayer() ? MessagesConfig.getMessage("commands.skip.header") : MessagesConfig.getMessage("commands.skip.header-other"));
        ...
        for (int i = 0; i < target.getQuests().size(); i++) {
            ...
            BasicQuestsPlugin.sendRawMessage(player, MessageFormat.format(MessagesConfig.getMessage("commands.skip.format"), quest.getInfo(i + 1, false, false),
                    targetNameArgument != null ? targetNameArgument + " " : "", i + 1, quest.getId()));
        }
        ...
        if (target.getPlayer() == player && !player.hasPermission("basicquests.admin.skip.unlimited")
