...
public class HelpCommand extends BasicQuestsCommand {
    ...
    public void evaluate(@NotNull BasicQuestsPlugin plugin, @NotNull CommandSender sender, @NotNull String alias, @NotNull @Unmodifiable List<String> params) {
        ...
        ChoiceFormat questAmountFormat = new ChoiceFormat(new double[]{1, 2},
                new String[]{MessagesConfig.getMessage("generic.quest.singular"), MessagesConfig.getMessage("generic.quest.plural"),});
        sb.append("\n")
                .append(MessageFormat.format(MessagesConfig.getMessage("commands.help.info"),
                        String.format("%s %s", Config.getQuestAmount(), questAmountFormat.format(Config.getQuestAmount())),
                        String.format("%s %s", Config.getSkipsPerDay(), questAmountFormat.format(Config.getSkipsPerDay()))));
