  ...
  public void evaluate(
    ...
    if (Config.isScoreboardDisabled()) {
      BasicQuestsPlugin.sendMessage(
          sender, MessagesConfig.getMessage("commands.scoreboard.disabled"));
      return;
    }
    ...
    if (params.isEmpty()) {
      if (QuestsScoreBoardManager.isBoardShowing(questPlayer.getPlayer())) {
        ...
        BasicQuestsPlugin.sendMessage(
            questPlayer.getPlayer(), MessagesConfig.getMessage("commands.scoreboard.hide"));
      } else {
        QuestsScoreBoardManager.show(questPlayer, false);
        BasicQuestsPlugin.sendMessage(
            questPlayer.getPlayer(), MessagesConfig.getMessage("commands.scoreboard.show"));
      }
