...
public class BasicQuestsCommandRouter implements CommandExecutor, TabCompleter {
  ...
  @Unmodifiable
  private static final List<BasicQuestsCommand> COMMANDS =
      ImmutableList.of(
          new HelpCommand(),
          new ListCommand(),
          new ReloadCommand(),
  ...
  @NotNull private final BasicQuestsPlugin plugin;

  @NotNull @Unmodifiable private final Map<String, BasicQuestsCommand> commands;
  ...
  public boolean onCommand(
    ...
    if (target == null) {
      BasicQuestsPlugin.sendMessage(
          sender,
          MessageFormat.format(MessagesConfig.getMessage("generic.unknown-command"), search));
      return true;
    }
  ...
  public List<String> onTabComplete(
    ...
    final Stream<String> targets =
        BasicQuestsCommand.filterByPermission(sender, commands.values().stream())
            .map(BasicQuestsCommand::getLabels)
