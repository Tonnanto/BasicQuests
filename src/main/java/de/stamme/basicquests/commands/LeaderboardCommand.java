...
public class LeaderboardCommand extends BasicQuestsCommand {
  ...
  void sendLeaderboardMessage(CommandSender sender) {
    StringBuilder message =
        new StringBuilder(MessagesConfig.getMessage("commands.leaderboard.header") + "\n");

    List<Map.Entry<UUID, Integer>> pointsLeaderboard =
    ...
    if (pointsLeaderboard.size() > 5) {
      BasicQuestsPlugin.sendRawMessage(
          sender, MessagesConfig.getMessage("commands.leaderboard.footer"));
    }
  }
  ...
  void sendFullLeaderboardMessage(CommandSender sender) {
    ...
    List<Map.Entry<UUID, Integer>> starsLeaderboard =
        ServerInfo.getInstance().getStarsLeaderboardSorted();
    Map<UUID, Integer> questCountMap = ServerInfo.getInstance().getQuestsLeaderboard();
  ...
  private String leaderboardString(String name, int pos, int stars, int questsCompleted) {
    if (name == null || name.isEmpty()) {
      return MessageFormat.format(
          MessagesConfig.getMessage("commands.leaderboard.empty-line"), pos);
    }
    return MessageFormat.format(
        MessagesConfig.getMessage("commands.leaderboard.line"),
        pos,
        name,
