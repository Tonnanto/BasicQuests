...
public class QuestsPlaceholderExpansion extends PlaceholderExpansion {
    ...
    if (params[0].equalsIgnoreCase("top")) {
      try {
        ...
        List<Map.Entry<UUID, Integer>> leaderboard =
            ServerInfo.getInstance().getStarsLeaderboardSorted();
        ...
        if (leaderboard.size() >= leaderboardPos) {
          ...
          String playerName =
              BasicQuestsPlugin.getPlugin()
                  .getServer()
                  .getOfflinePlayer(leaderboardEntry.getKey())
                  .getName();
    ...
    try {
      ...
      if (params.length == 1) {
        ...
        if (params[1].equalsIgnoreCase("reward")) {
          // %quests_1_reward_1%
          String[] rewardLines =
              questRewardLines(questPlayer.getQuests().get(questIndex).getReward());
  ...
  /**
   * Splits the title of a quest + the progress status into 4 lines of 14 or fewer characters Built
   * to display quests on signs.
   *
  ...
  private String[] questTitleLines(Quest quest) {
    ...
    String progressString =
        MessageFormat.format(
            MessagesConfig.getMessage("placeholder.quest-progress"), quest.getProgressString());
      ...
      case ITEM:
        ...
        if (itemRewardLines.length > 3) {
          rewardLines[1] +=
              " "
                  + MessageFormat.format(
                      MessagesConfig.getMessage("placeholder.more"), (itemRewardLines.length - 3));
        }
  ...
  private String leaderboardString(String name, int pos, int stars) {
    if (name == null || name.isEmpty()) {
      return MessageFormat.format(
          MessagesConfig.getMessage("placeholder.leaderboard.empty-line"), pos);
    }
    return MessageFormat.format(
        MessagesConfig.getMessage("placeholder.leaderboard.line"),
        pos,
        name,
        StringFormatter.starString(stars, true));
  }
}
