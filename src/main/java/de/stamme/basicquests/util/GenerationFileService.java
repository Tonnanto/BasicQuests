...
public class GenerationFileService {
    ...
    private void loadDefaultGenerationFiles() {
        ...
        for (QuestType questType : QuestType.values()) {
            ...
            if (configFileContent == null || isOutdated(configFileContent)) {
                savedFiles++;
                migrateGenerationFile(questGenerationBasePath + questType.name().toLowerCase() + ".yml",
        ...
        for (ItemRewardType itemRewardType : ItemRewardType.values()) {
            ...
            if (configFileContent == null || isOutdated(configFileContent)) {
                savedFiles++;
                migrateGenerationFile(itemRewardGenerationBasePath + itemRewardType.name().toLowerCase() + ".yml",
                        "quest_generation/item_reward_generation/" + itemRewardType.name().toLowerCase() + ".yml");
            }
            try {
    ...
    private String readFile(File file) {
        ...
        try {
            return Files.readAllLines(file.toPath()).stream().map(String::valueOf).collect(Collectors.joining("\n"));
        } catch (IOException e) {
    ...
    /**
     * Replaces old generation file with updated file of a newer version. Replaces all new values
     * that were also present in the old version with their past values.
     *
    ...
    private void migrateGenerationFile(String filePath, String newResourcePath) {
        try {
            ...
            for (GenerationOption option : newConfigOptions) {
                Optional<GenerationOption> matchingOldOption = oldConfigOptions.stream().filter(o -> o.getName().equalsIgnoreCase(option.getName()))
                        .findFirst();
