...
public class GenerationFileService {
  private static final String questGenerationBasePath =
      BasicQuestsPlugin.getPlugin().getDataFolder()
          + File.separator
          + "quest_generation"
          + File.separator;
  private static final String itemRewardGenerationBasePath =
      questGenerationBasePath + File.separator + "item_reward_generation" + File.separator;
  ...
  /**
   * 1. Checks if all configuration files exist and are up-to-date. 2. Saves default config files
   * that are missing or outdated. 3. Loads configurations into memory.
   */
  private void loadDefaultGenerationFiles() {
    ...
    if (questTypesConfigContent == null || isOutdated(questTypesConfigContent)) {
      savedFiles++;
      migrateGenerationFile(
          questGenerationBasePath + "quest_types.yml", "quest_generation/quest_types.yml");
    }
    try {
    ...
    for (QuestType questType : QuestType.values()) {
      ...
      if (configFileContent == null || isOutdated(configFileContent)) {
        savedFiles++;
        migrateGenerationFile(
            questGenerationBasePath + questType.name().toLowerCase() + ".yml",
            "quest_generation/" + questType.name().toLowerCase() + ".yml");
      }
      try {
    ...
    for (ItemRewardType itemRewardType : ItemRewardType.values()) {
      File configFile =
          new File(itemRewardGenerationBasePath + itemRewardType.name().toLowerCase() + ".yml");
      String configFileContent = readFile(configFile);
      if (configFileContent == null || isOutdated(configFileContent)) {
        savedFiles++;
        migrateGenerationFile(
            itemRewardGenerationBasePath + itemRewardType.name().toLowerCase() + ".yml",
            "quest_generation/item_reward_generation/"
                + itemRewardType.name().toLowerCase()
                + ".yml");
      }
      try {
    ...
    if (savedFiles != 0) {
      BasicQuestsPlugin.log(
          "Created " + savedFiles + " quest generation config files at " + questGenerationBasePath);
    }
  }
  ...
  /**
   * Checks whether a config file is outdated. Each config file contains a version string with the
   * version the file has been generated for.
   *
  ...
  /**
   * Replaces old generation file with updated file of a newer version. Replaces all new values that
   * were also present in the old version with their past values.
   *
  ...
  private void migrateGenerationFile(String filePath, String newResourcePath) {
    try {
      ...
      for (GenerationOption option : newConfigOptions) {
        Optional<GenerationOption> matchingOldOption =
            oldConfigOptions.stream()
      ...
      if (!newConfigOptions.isEmpty()) {
        newConfiguration.set(
            "options",
            newConfigOptions.stream().map(GenerationOption::toMap).collect(Collectors.toList()));
      }
  ...
  public GenerationConfig getConfigForQuestType(QuestType questType) {
    if (configForQuestType.get(questType) != null) return configForQuestType.get(questType).clone();
  ...
  private List<GenerationOption> getOptionsFromOptionList(List<?> optionList) {
    ...
    for (Object generationOption : optionList) {
      if (!(generationOption instanceof LinkedHashMap<?, ?>)) {
        BasicQuestsPlugin.log(
            Level.SEVERE, "Could not parse from generation file: " + generationOption);
        continue;
      }
