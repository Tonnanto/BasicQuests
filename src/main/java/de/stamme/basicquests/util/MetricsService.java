...
public class MetricsService {
  ...
  public static void setUpMetrics() {
    ...
    metrics.addCustomChart(
        new SimplePie(
            "economy", () -> (BasicQuestsPlugin.getEconomy() != null) ? "true" : "false"));

    // EssentialsXDiscord Pie Chart
    metrics.addCustomChart(
        new SimplePie(
            "uses_essentialsx_discord",
            () -> String.valueOf(BasicQuestsPlugin.usingEssentialsDiscord)));
        ...
        new SimplePie(
            ...
            () -> {
              ...
              if (BasicQuestsPlugin.getEconomy() != null && Config.moneyRewards())
                list.add("Money");
              if (Config.itemRewards()) list.add("Items");
              if (Config.xpRewards()) list.add("XP");
    ...
    metrics.addCustomChart(
        new SingleLineChart(
            "quests_completed", () -> ServerInfo.getInstance().getCompletedQuests().size()));

    // quests skipped line chart
    metrics.addCustomChart(
        new SingleLineChart(
            "quests_skipped", () -> ServerInfo.getInstance().getSkippedQuests().size()));

    // quest type completed advanced pie chart
    metrics.addCustomChart(
        new DrilldownPie(
            "type_of_completed_quests_drilldown",
            () -> {
    ...
    metrics.addCustomChart(
        new DrilldownPie(
            "type_of_skipped_quests_drilldown",
            () -> {
              Map<String, Map<String, Integer>> valueMap = new HashMap<>();
    ...
    metrics.addCustomChart(
        new SimplePie("quest_amount", () -> String.valueOf(Config.getQuestAmount())));

    // reward-factor pie chart
    metrics.addCustomChart(
        new SimplePie("reward_factor", () -> df.format(Config.getRewardFactor())));

    // quantity-factor pie chart
    metrics.addCustomChart(
        new SimplePie("quantity_factor", () -> df.format(Config.getQuantityFactor())));

    // skips per day pie chart
    metrics.addCustomChart(
        new SimplePie("skips_per_day", () -> df.format(Config.getSkipsPerDay())));

    // broadcast-on-quest-complete pie chart
    metrics.addCustomChart(
        new SimplePie(
            "broadcast_on_complete", () -> String.valueOf(Config.broadcastOnQuestCompletion())));

    // sound-on-quest-complete pie chart
    metrics.addCustomChart(
        new SimplePie("sound_on_complete", () -> String.valueOf(Config.soundOnQuestCompletion())));
    ...
    metrics.addCustomChart(
        new SimplePie(
            "increase_quantities_with_playtime",
            () -> String.valueOf(Config.increaseAmountByPlaytime())));
    ...
    metrics.addCustomChart(
        new SimplePie(
            "playtime_factor",
            () ->
                df.format(Config.minPlaytimeFactor())
                    + " -> "
                    + df.format(Config.maxPlaytimeFactor())
                    + " (at "
                    + df.format(Config.maxPlaytimeHours())
                    + " hours)"));

    // duplicate quest chance pie chart
    metrics.addCustomChart(
        new SimplePie("duplicate_quest_chance", () -> df.format(Config.duplicateQuestChance())));

    // disable scoreboard pie chart
    metrics.addCustomChart(
        new SimplePie("disable_scoreboard", () -> String.valueOf(Config.isScoreboardDisabled())));

    // show-scoreboard-per-default pie chart
    metrics.addCustomChart(
        new SimplePie(
            "show_scoreboard_per_default",
            () -> String.valueOf(Config.showScoreboardPerDefault())));

    // uses-custom-messages pie chart
    metrics.addCustomChart(
        new SimplePie(
            "uses_custom_messages", () -> String.valueOf(MessagesConfig.usesCustomMessages())));
  }
}
