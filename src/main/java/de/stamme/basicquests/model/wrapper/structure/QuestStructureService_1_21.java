...
public class QuestStructureService_1_21 extends QuestStructureService {
    ...
    public @Nullable Location findStructureNearLocation(
        QuestStructureType structureType,
        ...
        List<Structure> matchingStructures = toStructure(structureType);
        if (matchingStructures == null || matchingStructures.isEmpty()) {
            return null;
        }
        ...
        for (Structure structure : matchingStructures) {
            ...
            double distance = Math.hypot(
                nearLocation.getX() - location.getX(),
                nearLocation.getZ() - location.getZ()
            );
    ...
    @Nullable
    private List<Structure> toStructure(QuestStructureType questStructureType) {
        switch (questStructureType) {
            ...
            // 🆕 1.21
            case TRIAL_CHAMBERS:
                return Collections.singletonList(Structure.TRIAL_CHAMBERS);

            // 🟡 istniało wcześniej, ale często pomijane
            case NETHER_FOSSIL:
                return Collections.singletonList(Structure.NETHER_FOSSIL);
