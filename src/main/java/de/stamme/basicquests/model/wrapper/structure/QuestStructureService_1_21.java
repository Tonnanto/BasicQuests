...
public class QuestStructureService_1_21 extends QuestStructureService {
    ...
    public @Nullable Location findStructureNearLocation(QuestStructureType structureType, Location nearLocation, World world) {
        ...
        for (Structure structure : matchingStructures) {
            ...
            double distance = Math
                    .sqrt(Math.pow(Math.abs(nearLocation.getX() - location.getX()), 2) + Math.pow(Math.abs(nearLocation.getZ() - location.getZ()), 2));
    ...
    private List<Structure> toSpigotStructure(QuestStructureType questStructureType) {
        switch (questStructureType) {
            case VILLAGE :
                return Arrays.asList(Structure.VILLAGE_PLAINS, Structure.VILLAGE_SAVANNA, Structure.VILLAGE_DESERT, Structure.VILLAGE_SNOWY,
            ...
            case RUINED_PORTAL :
                return Arrays.asList(Structure.RUINED_PORTAL, Structure.RUINED_PORTAL_DESERT, Structure.RUINED_PORTAL_JUNGLE, Structure.RUINED_PORTAL_MOUNTAIN,
                        Structure.RUINED_PORTAL_NETHER, Structure.RUINED_PORTAL_OCEAN, Structure.RUINED_PORTAL_SWAMP);
