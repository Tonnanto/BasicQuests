...
// Extended and upgraded potion variants are now their own PotionTypes rather than a flag in the
// item's metadata
public class QuestPotionService_1_21 extends QuestPotionService {
  @Override
  @Nullable
  public ItemStack getPotionItemStack(
    ...
    try {
      ...
      BasicQuestsPlugin.log(
          Level.INFO, String.format("PotionType '%s' does not exist in this version.", potionName));
      return null;
    }
    ...
    if (!(itemMeta instanceof PotionMeta)) {
      BasicQuestsPlugin.log(
          Level.SEVERE,
          "Could not find PotionData for item with Material: " + item.getType().name());
      return null;
    }
  ...
  /**
   * Takes a base potion type and converts it into a new specific potion type based on whether its
   * extended (LONG) or upgraded (STRONG) These specific potion types were added in 1.20
   *
  ...
  PotionType getPotionType(PotionType basePotionType, boolean extended, boolean upgraded) {
    ...
    switch (basePotionType) {
      case NIGHT_VISION:
        if (extended) return LONG_NIGHT_VISION;
        return NIGHT_VISION;
      case INVISIBILITY:
        if (extended) return LONG_INVISIBILITY;
        return INVISIBILITY;
      case LEAPING:
        if (extended) return LONG_LEAPING;
        return STRONG_LEAPING;
      case FIRE_RESISTANCE:
        if (extended) return LONG_FIRE_RESISTANCE;
        return FIRE_RESISTANCE;
      case SWIFTNESS:
        if (extended) return LONG_SWIFTNESS;
        return STRONG_SWIFTNESS;
      case SLOWNESS:
        if (extended) return LONG_SLOWNESS;
        return STRONG_SLOWNESS;
      case WATER_BREATHING:
        if (extended) return LONG_WATER_BREATHING;
        return WATER_BREATHING;
      case HEALING:
        if (extended) return HEALING;
        return STRONG_HEALING;
      case HARMING:
        if (extended) return HARMING;
        return STRONG_HARMING;
      case POISON:
        if (extended) return LONG_POISON;
        return STRONG_POISON;
      case REGENERATION:
        if (extended) return LONG_REGENERATION;
        return STRONG_REGENERATION;
      case STRENGTH:
        if (extended) return LONG_STRENGTH;
        return STRONG_STRENGTH;
      case WEAKNESS:
        if (extended) return LONG_WEAKNESS;
        return WEAKNESS;
      case TURTLE_MASTER:
        if (extended) return LONG_TURTLE_MASTER;
        return STRONG_TURTLE_MASTER;
      case SLOW_FALLING:
        if (extended) return LONG_SLOW_FALLING;
        return SLOW_FALLING;
      default:
        return basePotionType;
    }
