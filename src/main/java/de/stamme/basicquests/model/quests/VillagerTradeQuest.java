...
public class VillagerTradeQuest extends Quest {
    ...
    String villagerName =
        (this.profession == Villager.Profession.NONE)
            ...
            : MinecraftLocaleConfig.getMinecraftName(
                profession.getKey().getKey(), "entity.minecraft.villager.");
    return goal > 1
        ? MessageFormat.format(
            MessagesConfig.getMessage("quests.villager-trade.plural"), villagerName, goal)
        : MessageFormat.format(
            MessagesConfig.getMessage("quests.villager-trade.singular"), villagerName);
  }

  @Override
  public String[] getOptionNames() {
    return new String[] {QuestType.VILLAGER_TRADE.name(), this.getOptionKey()};
  }
  ...
  public String getOptionKey() {
    return (profession == Villager.Profession.NONE)
        ? "VILLAGER"
        : profession.getKey().getKey().toUpperCase();
  }
}
