...
public class ChopWoodQuest extends Quest {
    ...
    if (getMaterialString() == null || getMaterialString().isEmpty()) {
      ...
      if (goal <= 1) {
        ...
        return MessageFormat.format(
            MessagesConfig.getMessage("quests.chop-wood.singular"), singularName);
      } else {
        String pluralName =
            MessagesConfig.getPluralName(getQuestType(), getOptionKey(), "block.minecraft.");
        return MessageFormat.format(
            MessagesConfig.getMessage("quests.chop-wood.plural"), goal, pluralName);
      }
  ...
  public String getOptionKey() {
    if (materialString != null && !materialString.isEmpty()) return materialString;
