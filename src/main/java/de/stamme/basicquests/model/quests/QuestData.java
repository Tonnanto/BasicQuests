...
public class QuestData implements Serializable {
    ...
    public Quest toQuest() {
        ...
        if (questType.equals(QuestType.BREAK_BLOCK.name())) {
            ...
            } else if (material.equalsIgnoreCase(Material.ACACIA_LOG.name()) || material.equalsIgnoreCase(Material.BIRCH_LOG.name()) || material.equalsIgnoreCase(Material.DARK_OAK_LOG.name()) || material.equalsIgnoreCase(Material.JUNGLE_LOG.name()) || material.equalsIgnoreCase(Material.OAK_LOG.name()) || material.equalsIgnoreCase(Material.SPRUCE_LOG.name())) {
    ...
    public boolean isInvalid() {
        ...
        boolean hasInvalidMoneyReward = (getReward().getRewardType() == RewardType.MONEY || getReward().getMoney().compareTo(BigDecimal.ZERO) > 0) && !Config.moneyRewards();

        boolean hasInvalidXpReward = (getReward().getRewardType() == RewardType.XP || getReward().getXp() > 0) && !Config.xpRewards();

        boolean hasInvalidItemReward = (getReward().getRewardType() == RewardType.ITEM || (getReward().getItems() != null && !getReward().getItems().isEmpty())) && !Config.itemRewards();
