...
public class QuestData implements Serializable {
  ...
  public Quest toQuest() {
    ...
    if (questType.equals(QuestType.BREAK_BLOCK.name())) {
      ...
      if (materialString != null && materialString.equalsIgnoreCase("LOG")) {
        ...
        try {
          ...
          BasicQuestsPlugin.log(
              Level.SEVERE, String.format("Material '%s' does not exist.", material));
        }
        // +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      } else {
        try {
      ...
      try {
        ...
        BasicQuestsPlugin.log(
            Level.SEVERE, String.format("EntityType '%s' does not exist.", entity));
      }
  ...
  public boolean isInvalid() {
    ...
    boolean hasInvalidMoneyReward =
        (getReward().getRewardType() == RewardType.MONEY
                || getReward().getMoney().compareTo(BigDecimal.ZERO) > 0)
            && !Config.moneyRewards();

    boolean hasInvalidXpReward =
        (getReward().getRewardType() == RewardType.XP || getReward().getXp() > 0)
            && !Config.xpRewards();
  ...
  public String toString() {
    return String.format(
        "Type: %s, goal: %s, count %s, reward %s, mat: %s, ent: %s, enc: %s",
        questType, goal, count, reward.getMoney(), material, entity, enchantment);
  }
