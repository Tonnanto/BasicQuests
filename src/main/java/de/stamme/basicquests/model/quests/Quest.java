...
public abstract class Quest {
    ...
    if (isCompleted()) {
      ...
      questPlayer.sendMessage(
          MessageFormat.format(
              MessagesConfig.getMessage("events.player.stars-gained"),
              StringFormatter.starString(getStarValue(), true)));
      questPlayer.sendMessage(MessagesConfig.getMessage("events.player.receive-reward"));
      questPlayer
          .getPlayer()
          .sendTitle(
              MessagesConfig.getMessage("events.player.quest-completed"), getName(), 10, 70, 20);
  ...
  public static void startProgressScheduler() {
    Bukkit.getScheduler()
        .runTaskTimer(
            ...
            () -> {
              for (Map.Entry<UUID, QuestPlayer> entry :
  ...
  public String getInfo(int questNumber, boolean withReward, boolean showPointsOnHover) {
    if (withReward) {
      return MessageFormat.format(
              MessagesConfig.getMessage(
                  showPointsOnHover ? "quest.format.hoverable" : "quest.format.raw"),
              String.valueOf(questNumber),
              getName(),
              getProgressString(),
              StringFormatter.starString(
                  getStarValue(), false) // value only used in "quest.format.hoverable" message
              )
          + getReward().toString()
          + "\n";
    }
  ...
  public String debugString() {
    return MessageFormat.format(
            "{0} ({1})", getName(), StringFormatter.starString(getStarValue(), false))
  ...
  public String getLeftString() {
    ...
    return MessageFormat.format(
        MessagesConfig.getMessage("quest.progress.remaining"), getGoal() - getCount());
  }
