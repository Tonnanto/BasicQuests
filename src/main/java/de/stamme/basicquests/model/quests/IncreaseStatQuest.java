...
public class IncreaseStatQuest extends Quest {
    ...
    public void checkForProgress(QuestPlayer questPlayer) {
        ...
        if (getStatistic() == Statistic.WALK_ONE_CM || getStatistic() == Statistic.SPRINT_ONE_CM) {
            questProgress = (questPlayer.getPlayer().getStatistic(Statistic.WALK_ONE_CM) + questPlayer.getPlayer().getStatistic(Statistic.SPRINT_ONE_CM))
                    - getStartValue();
        } else {
    ...
    public String getName() {
        switch (statistic) {
            case WALK_ONE_CM :
            case SPRINT_ONE_CM :
                return MessageFormat.format(MessagesConfig.getMessage("quests.increase-stat.run"), getDistanceGoal());
            case SWIM_ONE_CM :
                return MessageFormat.format(MessagesConfig.getMessage("quests.increase-stat.swim"), getDistanceGoal());
            case BOAT_ONE_CM :
                return MessageFormat.format(MessagesConfig.getMessage("quests.increase-stat.boat"), getDistanceGoal());
            case HORSE_ONE_CM :
                return MessageFormat.format(MessagesConfig.getMessage("quests.increase-stat.horse"), getDistanceGoal());
            case PIG_ONE_CM :
                return MessageFormat.format(MessagesConfig.getMessage("quests.increase-stat.pig"), getDistanceGoal());
            case STRIDER_ONE_CM :
                return MessageFormat.format(MessagesConfig.getMessage("quests.increase-stat.strider"), getDistanceGoal());
            case MINECART_ONE_CM :
                return MessageFormat.format(MessagesConfig.getMessage("quests.increase-stat.minecart"), getDistanceGoal());
            case RECORD_PLAYED :
            ...
            default :
                return MessageFormat.format(MessagesConfig.getMessage("quests.increase-stat.generic"), StringFormatter.format(statistic.name()), getGoal());
        }
    }
