...
public class EnchantItemQuest extends Quest {
  ...
  public String getName() {
    ...
    if (goal <= 1) {
      // Enchant 1 item
      String singularName =
          MinecraftLocaleConfig.getMinecraftName(getOptionKey(), "item.minecraft.");
      if (!withEnchantment) {
        return MessageFormat.format(
            MessagesConfig.getMessage("quests.enchant-item.any.singular"), singularName);
      }
      String enchantmentName = StringFormatter.enchantmentName(getEnchantment());
      if (!withLevel) {
        return MessageFormat.format(
            MessagesConfig.getMessage("quests.enchant-item.singular.withoutLevel"),
            singularName,
            enchantmentName);
      }
      ...
      return MessageFormat.format(
          MessagesConfig.getMessage("quests.enchant-item.singular.generic"),
          singularName,
          enchantmentName,
          enchantmentLevel);
      ...
      String pluralName =
          MessagesConfig.getPluralName(getQuestType(), getOptionKey(), "item.minecraft.");
      if (!withEnchantment) {
        return MessageFormat.format(
            MessagesConfig.getMessage("quests.enchant-item.any.plural"), goal, pluralName);
      }
      String enchantmentName = StringFormatter.enchantmentName(getEnchantment());
      if (!withLevel) {
        return MessageFormat.format(
            MessagesConfig.getMessage("quests.enchant-item.plural.withoutLevel"),
            goal,
            pluralName,
            enchantmentName);
      }
      ...
      return MessageFormat.format(
          MessagesConfig.getMessage("quests.enchant-item.plural.generic"),
          goal,
          pluralName,
          enchantmentName,
          enchantmentLevel);
