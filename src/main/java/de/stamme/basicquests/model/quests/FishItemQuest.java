...
public class FishItemQuest extends Quest {
  ...
  public boolean itemMatches(Item item) {
    ...
    List<String> treasures =
        Arrays.asList("BOW", "ENCHANTED_BOOK", "NAME_TAG", "NAUTILUS_SHELL", "SADDLE");
  ...
  public String getName() {
    ...
    if (option == Option.ANY_FISH) {
      if (goal <= 1) {
        ...
        return MessageFormat.format(
            MessagesConfig.getMessage("quests.fish-item.any-fish.plural"), goal);
      }
    }
    ...
    if (option == Option.ANY_TREASURE) {
      if (goal <= 1) {
        ...
        return MessageFormat.format(
            MessagesConfig.getMessage("quests.fish-item.any-treasure.plural"), goal);
      }
    }
    ...
    if (option == Option.ANY_ITEM) {
      if (goal <= 1) {
        ...
        return MessageFormat.format(
            MessagesConfig.getMessage("quests.fish-item.any-item.plural"), goal);
      }
    }
    ...
    if (goal <= 1) {
      String singularName =
          MinecraftLocaleConfig.getMinecraftName(
              getOptionKey(), "entity.minecraft.", "item.minecraft.");
      return MessageFormat.format(
          MessagesConfig.getMessage("quests.fish-item.singular"), singularName);
    } else {
      String pluralName =
          MessagesConfig.getPluralName(
              getQuestType(), getOptionKey(), "entity.minecraft.", "item.minecraft.");
      return MessageFormat.format(
          MessagesConfig.getMessage("quests.fish-item.plural"), goal, pluralName);
    }
  }
