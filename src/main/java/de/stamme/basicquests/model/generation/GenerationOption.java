...
public class GenerationOption {
    ...
    public void updateWith(GenerationOption oldOption) {
        ...
        if (options != null && !options.isEmpty() && oldOption.options != null && !oldOption.options.isEmpty()) {
            for (GenerationOption option : options) {
                Optional<GenerationOption> matchingOldOption = oldOption.options.stream().filter(o -> o.getName().equalsIgnoreCase(option.getName()))
                        .findFirst();
    ...
    public Map<String, Object> toMap() {
        ...
        if (value_base != 0) {
            map.put("value_base", value_base);
        }
        if (value_per_unit != 1) {
            map.put("value_per_unit", value_per_unit);
        }
        if (radius != 60) {
        ...
        if (advancements != null) {
            map.put("advancements", advancements);
        }
        if (questTypes != null) {
        ...
        if (options != null && !options.isEmpty()) {
            map.put("options", options.stream().map(GenerationOption::toMap).collect(Collectors.toList()));
        }
    ...
    public void setValueBase(double value_base) {
    ...
    public void setValuePerUnit(double value_per_unit) {
    ...
    public String toString() {
        return "DecisionObject{" + "name='" + name + '\'' + ", value=" + value + ", weight=" + weight + ", valueBase=" + value_base + ", valuePerUnit="
                + value_per_unit + ", radius=" + radius + ", min=" + min + ", max=" + max + ", step=" + step + ", options=" + options + ", advancements="
                + advancements + ", questTypes=" + questTypes + ", variants=" + variants + '}';
    }
}
