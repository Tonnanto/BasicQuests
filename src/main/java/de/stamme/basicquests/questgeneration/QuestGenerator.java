...
public class QuestGenerator {
  ...
  public static QuestGenerator getInstance() {
    if (instance == null) instance = new QuestGenerator();
    return instance;
  }
  ...
  public GenerationOption decide(List<GenerationOption> objects) {
    double x = 0;
    double tot =
        objects.stream().map(GenerationOption::getWeight).mapToDouble(Double::doubleValue).sum();
  ...
  public GenerationOption decide(List<GenerationOption> options, QuestPlayer questPlayer) {
    ...
    for (GenerationOption option : options) {
      ...
      // set Options weight to 0 if a required advancement has not been made (eg.
      // "story/mine_diamond")
  ...
  public Quest generate(QuestPlayer questPlayer) throws QuestGenerationException {
    ...
    GenerationConfig generationConfig =
        GenerationFileService.getInstance().getQuestTypeGenerationConfig();
    ...
    try {
      ...
      throw new QuestGenerationException(
          String.format("QuestType '%s' does not exist.", questTypeOption.getName()));
    }

    switch (questType) {
      case BREAK_BLOCK:
        quest =
            generateBreakBlockQuest(
                questPlayer, questTypeOption.getValue() * reward_factor, amount_factor);
        break;
      case MINE_BLOCK:
        quest =
            generateMineBlockQuest(
                questPlayer, questTypeOption.getValue() * reward_factor, amount_factor);
        break;
      case HARVEST_BLOCK:
        quest =
            generateHarvestBlockQuest(
                questPlayer, questTypeOption.getValue() * reward_factor, amount_factor);
        break;
      case ENCHANT_ITEM:
        quest =
            generateEnchantItemQuest(
                questPlayer, questTypeOption.getValue() * reward_factor, amount_factor);
        break;
      case KILL_ENTITY:
        quest =
            generateKillEntityQuest(
                questPlayer, questTypeOption.getValue() * reward_factor, amount_factor);
        break;
      case GAIN_LEVEL:
        quest =
            generateGainLevelQuest(
                questPlayer, questTypeOption.getValue() * reward_factor, amount_factor);
        break;
      case REACH_LEVEL:
      ...
      case CHOP_WOOD:
        quest =
            generateChopWoodQuest(
                questPlayer, questTypeOption.getValue() * reward_factor, amount_factor);
        break;
      case VILLAGER_TRADE:
        quest =
            generateVillagerTradeQuest(
                questPlayer, questTypeOption.getValue() * reward_factor, amount_factor);
        break;
      case FISH_ITEM:
        quest =
            generateFishItemQuest(
                questPlayer, questTypeOption.getValue() * reward_factor, amount_factor);
        break;
      case INCREASE_STAT:
        quest =
            generateIncreaseStatQuest(
                questPlayer, questTypeOption.getValue() * reward_factor, amount_factor);
        break;
    }
  ...
  Quest generateBreakBlockQuest(QuestPlayer questPlayer, double reward_factor, double amount_factor)
      throws QuestGenerationException {
    GenerationConfig generationConfig =
        GenerationFileService.getInstance().getConfigForQuestType(QuestType.BREAK_BLOCK);
    ...
    if (materialToBreak == null) {
      BasicQuestsPlugin.log(
          Level.INFO,
          String.format("Material '%s' does not exist in this version.", materialOption.getName()));
  ...
  Quest generateMineBlockQuest(QuestPlayer questPlayer, double reward_factor, double amount_factor)
      throws QuestGenerationException {
    GenerationConfig generationConfig =
        GenerationFileService.getInstance().getConfigForQuestType(QuestType.MINE_BLOCK);
  ...
  Quest generateHarvestBlockQuest(
      QuestPlayer questPlayer, double reward_factor, double amount_factor)
      throws QuestGenerationException {
    GenerationConfig generationConfig =
        GenerationFileService.getInstance().getConfigForQuestType(QuestType.HARVEST_BLOCK);
  ...
  Quest generateChopWoodQuest(QuestPlayer questPlayer, double reward_factor, double amount_factor)
      throws QuestGenerationException {
    GenerationConfig generationConfig =
        GenerationFileService.getInstance().getConfigForQuestType(QuestType.CHOP_WOOD);
    ...
    if (!woodOption.getName().equalsIgnoreCase("LOG")) {
      ...
      if (woodToChop == null) {
        BasicQuestsPlugin.log(
            Level.INFO,
            String.format("Material '%s' does not exist in this version.", woodOption.getName()));
        return generate(questPlayer);
      }
    }
    ...
    if (woodOption
        .getName()
        .equalsIgnoreCase("LOG")) { // General Log quest that accepts all kind of logs
  ...
  Quest generateKillEntityQuest(QuestPlayer questPlayer, double reward_factor, double amount_factor)
      throws QuestGenerationException {
    GenerationConfig generationConfig =
        GenerationFileService.getInstance().getConfigForQuestType(QuestType.KILL_ENTITY);
    ...
    try {
      ...
      BasicQuestsPlugin.log(
          Level.INFO,
          String.format("Entity '%s' does not exist in this version.", entityOption.getName()));
      return generate(questPlayer);
    }
  ...
  Quest generateEnchantItemQuest(
      QuestPlayer questPlayer, double reward_factor, double amount_factor)
      throws QuestGenerationException {
    GenerationConfig generationConfig =
        GenerationFileService.getInstance().getConfigForQuestType(QuestType.ENCHANT_ITEM);
    ...
    if (materialOption.getOptions() != null) { // When Enchantments are available
      ...
      NamespacedKey enchantmentKey =
          NamespacedKey.minecraft(enchantmentOption.getName().toLowerCase());
      ...
      if (enchantment != null) {
        ...
        double value =
            materialOption.getValue(amountToEnchant)
                * enchantmentOption.getValue(enchantmentLevel)
                * reward_factor;
        Reward reward = generateReward(QuestType.ENCHANT_ITEM, value, questPlayer);

        Quest quest =
            new EnchantItemQuest(
                itemToEnchant, enchantment, enchantmentLevel, amountToEnchant, reward);
        quest.setValue(value);
        return quest;
      }
    }
  ...
  Quest generateReachLevelQuest(QuestPlayer questPlayer, double reward_factor)
      throws QuestGenerationException {
    GenerationConfig generationConfig =
        GenerationFileService.getInstance().getConfigForQuestType(QuestType.REACH_LEVEL);
  ...
  Quest generateGainLevelQuest(QuestPlayer questPlayer, double reward_factor, double amount_factor)
      throws QuestGenerationException {
    GenerationConfig generationConfig =
        GenerationFileService.getInstance().getConfigForQuestType(QuestType.GAIN_LEVEL);
  ...
  Quest generateFindStructureQuest(QuestPlayer questPlayer, double reward_factor)
      throws QuestGenerationException {
    GenerationConfig generationConfig =
        GenerationFileService.getInstance().getConfigForQuestType(QuestType.FIND_STRUCTURE);
    ...
    if (structureToFind == null) {
      BasicQuestsPlugin.log(
          Level.INFO,
          String.format(
              "QuestStructureType '%s' is not available in this version.",
              structureOption.getName()));
      return generate(questPlayer);
    }
  ...
  Quest generateVillagerTradeQuest(
      QuestPlayer questPlayer, double reward_factor, double amount_factor)
      throws QuestGenerationException {
    GenerationConfig generationConfig =
        GenerationFileService.getInstance().getConfigForQuestType(QuestType.VILLAGER_TRADE);
    ...
    try {
      NamespacedKey villagerProfessionKey =
          NamespacedKey.minecraft(professionOption.getName().toLowerCase());
      professionToTradeWith = Registry.VILLAGER_PROFESSION.get(villagerProfessionKey);
      ;
    } catch (IllegalArgumentException exception) {
      // If Profession was not found
      BasicQuestsPlugin.log(
          Level.SEVERE,
          String.format("Profession '%s' does not exist.", professionOption.getName()));
      return generate(questPlayer);
    }
  ...
  Quest generateFishItemQuest(QuestPlayer questPlayer, double reward_factor, double amount_factor)
      throws QuestGenerationException {
    GenerationConfig generationConfig =
        GenerationFileService.getInstance().getConfigForQuestType(QuestType.FISH_ITEM);
    ...
    if (materialToFish == null) {
      ...
      try {
        ...
        BasicQuestsPlugin.log(
            Level.INFO,
            String.format(
                "Material '%s' does not exist in this version.", materialOption.getName()));
        return generate(questPlayer);
      }
    }
  ...
  Quest generateIncreaseStatQuest(
      QuestPlayer questPlayer, double reward_factor, double amount_factor)
      throws QuestGenerationException {
    GenerationConfig generationConfig =
        GenerationFileService.getInstance().getConfigForQuestType(QuestType.INCREASE_STAT);
    ...
    try {
      ...
      BasicQuestsPlugin.log(
          Level.SEVERE, String.format("Statistic '%s' does not exist.", statisticOption.getName()));
      return generate(questPlayer);
    }
    ...
    int startValue;
    if (statisticToIncrease == Statistic.WALK_ONE_CM
        || statisticToIncrease == Statistic.SPRINT_ONE_CM) {
      startValue =
  ...
  /**
   * Returns an appropriate amount for a given DecisionObject. p.e: The amount of coal ores to break
   * in a quest
   *
   * @param generationOption the GenerationOption to generate an amount for
   * @param generationConfig the GenerationConfig that contains default values for min and max
   *     amount values
   * @param multiplier the multiplier for the amount
   * @return the generated amount
   */
  public int generateAmount(
      GenerationOption generationOption, GenerationConfig generationConfig, double multiplier) {
  ...
  protected double getPlaytimeAmountFactor(Player player) {
    ...
    double factor =
        start_factor + (max_factor - start_factor) * ((double) hours_played / max_amount_hours);
  ...
  /**
   * Chooses a reward type based on the config values and generates a reward for the given quest
   * accordingly.
   *
  ...
  private Reward generateReward(QuestType questType, double questValue, QuestPlayer questPlayer) {
    ...
    if (Config.xpRewards()) rewardTypes.add(RewardType.XP);
    if (Config.itemRewards() || rewardTypes.isEmpty()) rewardTypes.add(0, RewardType.ITEM);

    double totalWeight =
        rewardTypes.stream().map(RewardType::getWeight).reduce(Double::sum).orElse(1.0);
    ...
    switch (selectedRewardType) {
      case ITEM:
        ...
        questPlayer.getQuests().stream()
            .map(Quest::getReward)
            .filter(reward -> reward.getMaterialNames() != null)
            .forEach(reward -> materialsInRewards.addAll(reward.getMaterialNames()));
