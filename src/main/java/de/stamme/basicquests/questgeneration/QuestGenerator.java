...
public class QuestGenerator {
    ...
    public GenerationOption decide(List<GenerationOption> objects) {
        double x = 0;
        double tot = objects.stream().map(GenerationOption::getWeight).mapToDouble(Double::doubleValue).sum();
    ...
    public Quest generate(QuestPlayer questPlayer) throws QuestGenerationException {
        ...
        double amountFactor = Config.getQuantityFactor();

        if (Config.increaseAmountByPlaytime()) {
            amountFactor *= getPlaytimeAmountFactor(questPlayer.getPlayer());
        }
        ...
        switch (questType) {
            case BREAK_BLOCK :
                quest = generateBreakBlockQuest(questPlayer, questTypeOption.getValue() * rewardFactor, amountFactor);
                break;
            case MINE_BLOCK :
                quest = generateMineBlockQuest(questPlayer, questTypeOption.getValue() * rewardFactor, amountFactor);
                break;
            case HARVEST_BLOCK :
                quest = generateHarvestBlockQuest(questPlayer, questTypeOption.getValue() * rewardFactor, amountFactor);
                break;
            case ENCHANT_ITEM :
                quest = generateEnchantItemQuest(questPlayer, questTypeOption.getValue() * rewardFactor, amountFactor);
                break;
            case KILL_ENTITY :
                quest = generateKillEntityQuest(questPlayer, questTypeOption.getValue() * rewardFactor, amountFactor);
                break;
            case GAIN_LEVEL :
                quest = generateGainLevelQuest(questPlayer, questTypeOption.getValue() * rewardFactor, amountFactor);
                break;
            case REACH_LEVEL :
                quest = generateReachLevelQuest(questPlayer, questTypeOption.getValue() * rewardFactor);
                break;
            case FIND_STRUCTURE :
                quest = generateFindStructureQuest(questPlayer, questTypeOption.getValue() * rewardFactor);
                break;
            case CHOP_WOOD :
                quest = generateChopWoodQuest(questPlayer, questTypeOption.getValue() * rewardFactor, amountFactor);
                break;
            case VILLAGER_TRADE :
                quest = generateVillagerTradeQuest(questPlayer, questTypeOption.getValue() * rewardFactor, amountFactor);
                break;
            case FISH_ITEM :
                quest = generateFishItemQuest(questPlayer, questTypeOption.getValue() * rewardFactor, amountFactor);
                break;
            case INCREASE_STAT :
    ...
    Quest generateReachLevelQuest(QuestPlayer questPlayer, double rewardFactor) throws QuestGenerationException {
        GenerationConfig generationConfig = GenerationFileService.getInstance().getConfigForQuestType(QuestType.REACH_LEVEL);

        int minReach = generationConfig.getDefaultMin();
        int maxReach = generationConfig.getDefaultMax();
        int stepReach = generationConfig.getDefaultStep();
        double valuePerXp = generationConfig.getValuePerUnit();
        ...
        if (minReach < maxReach) {
            ...
            double value = rewardFactor * valuePerXp * xpRequired;
    ...
    Quest generateGainLevelQuest(QuestPlayer questPlayer, double reward_factor, double amount_factor) throws QuestGenerationException {
        ...
        int minGain = generationConfig.getDefaultMin();
        int maxGain = generationConfig.getDefaultMax();
        int stepGain = generationConfig.getDefaultStep();
        double valuePerLevel = generationConfig.getValuePerUnit();

        int amountToGain = generateAmount(minGain, maxGain, stepGain, amount_factor);

        double value = reward_factor * valuePerLevel * amountToGain;
    ...
    Quest generateVillagerTradeQuest(QuestPlayer questPlayer, double reward_factor, double amount_factor) throws QuestGenerationException {
        ...
        try {
            NamespacedKey villagerProfessionKey = NamespacedKey.minecraft(professionOption.getName().toLowerCase());
    ...
    Quest generateIncreaseStatQuest(QuestPlayer questPlayer, double reward_factor, double amount_factor) throws QuestGenerationException {
        ...
        if (statisticToIncrease == Statistic.WALK_ONE_CM || statisticToIncrease == Statistic.SPRINT_ONE_CM) {
            startValue = questPlayer.getPlayer().getStatistic(Statistic.WALK_ONE_CM) + questPlayer.getPlayer().getStatistic(Statistic.SPRINT_ONE_CM);
        } else {
    ...
    /**
     * Returns an appropriate amount for a given DecisionObject. p.e: The amount of coal ores to
     * break in a quest
     *
    ...
    public int generateAmount(GenerationOption generationOption, GenerationConfig generationConfig, double multiplier) {
        ...
        if (generationOption.getMax() > 0 && generationOption.getStep() > 0) {
            ...
            min = generationConfig.getDefaultMin();
            max = generationConfig.getDefaultMax();
            step = generationConfig.getDefaultStep();
        }
    ...
    protected double getPlaytimeAmountFactor(Player player) {
        ...
        int hoursPlayed = ticksPlayed / 20 / 60 / 60;

        double startFactor = config.getDouble("start-factor");
        double maxFactor = config.getDouble("max-factor");
        double maxAmountHours = config.getDouble("max-amount-hours");

        double factor = startFactor + (maxFactor - startFactor) * ((double) hoursPlayed / maxAmountHours);
    ...
    private Reward generateReward(QuestType questType, double questValue, QuestPlayer questPlayer) {
        ...
        switch (selectedRewardType) {
            case ITEM :
                ...
                questPlayer.getQuests().stream().map(Quest::getReward).filter(reward -> reward.getMaterialNames() != null)
