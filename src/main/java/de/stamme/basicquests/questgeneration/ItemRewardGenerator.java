...
public class ItemRewardGenerator {
  ...
  public static GenerationOption decide(
      List<GenerationOption> options,
      QuestType questType,
      double maxValue,
      List<String> rewardsInPlayerQuests) {

    for (GenerationOption option : options) {
      // Remove options where minValue > maxRewardValue
      double minValue;
      if (option.getVariants() != null) {
        Optional<Map.Entry<String, Double>> lowestMat =
            option.getVariants().entrySet().stream().min(Map.Entry.comparingByValue());
        minValue =
            option.getMin()
                * (lowestMat.isPresent() ? lowestMat.get().getValue() : option.getValue());
      } else {
      ...
      } else if (rewardsInPlayerQuests != null
          && rewardsInPlayerQuests.contains(option.getName())) {
  ...
  public static Reward generate(
      QuestType questType, double questValue, List<String> rewardsInPlayerQuests) {
    ...
    GenerationConfig toolRewardsList =
        GenerationFileService.getInstance().getConfigForItemRewardType(ItemRewardType.TOOL_REWARDS);
    GenerationConfig armorRewardsList =
        GenerationFileService.getInstance()
            .getConfigForItemRewardType(ItemRewardType.ARMOR_REWARDS);
    GenerationConfig enchantmentRewardsList =
        GenerationFileService.getInstance()
            .getConfigForItemRewardType(ItemRewardType.ENCHANTMENT_REWARDS);
    GenerationConfig resourceRewardsList =
        GenerationFileService.getInstance()
            .getConfigForItemRewardType(ItemRewardType.RESOURCE_REWARDS);
    GenerationConfig foodRewardsList =
        GenerationFileService.getInstance().getConfigForItemRewardType(ItemRewardType.FOOD_REWARDS);
    GenerationConfig potionRewardsList =
        GenerationFileService.getInstance()
            .getConfigForItemRewardType(ItemRewardType.POTION_REWARDS);
    GenerationConfig smithingTemplateRewardsList =
        GenerationFileService.getInstance()
            .getConfigForItemRewardType(ItemRewardType.SMITHING_TEMPLATE_REWARDS);
    GenerationConfig otherItemRewardsList =
        GenerationFileService.getInstance()
            .getConfigForItemRewardType(ItemRewardType.OTHER_ITEM_REWARDS);
    ...
    do {
      ...
      GenerationOption materialDO =
          decide(generationOptions, questType, maxValue, rewardsInPlayerQuests);
      if (materialDO == null) break;

      RewardItem rewardItem;

      if (toolRewardsList.getOptions().contains(materialDO)
          || armorRewardsList.getOptions().contains(materialDO)) {
  ...
  private static RewardItem getReward(GenerationOption materialOption, double maxValue) {
    ...
    while (getValue(materialOption.getValue(), amount + materialOption.getStep()) < maxValue
  ...
  @Nullable
  private static RewardItem getToolArmorReward(
      GenerationOption materialOption, double minValue, double maxValue) {
    ...
    if (materialOption.getVariants() != null) {
      Optional<Map.Entry<String, Double>> lowestMat =
          materialOption.getVariants().entrySet().stream().min(Map.Entry.comparingByValue());
      materialString = lowestMat.isPresent() ? lowestMat.get().getKey() : "";
      materialValue =
          lowestMat.isPresent() ? lowestMat.get().getValue() : materialOption.getValue();
    } else {
    ...
    if (!materialString.isEmpty() || enchantment != null) {
      while (itemValue < minValue
          && (!materialString.equalsIgnoreCase("NETHERITE")
        ...
        if (oldItemValue == itemValue) break;
      }
    }
    ...
    if (!materialString.isEmpty()) materialString += "_" + materialOption.getName();
    else materialString = materialOption.getName();
  ...
  @Nullable
  private static RewardItem getEnchantmentReward(
      GenerationOption enchantmentOption, double maxValue) {
    ...
    if (enchantment != null) {
      ...
      BasicQuestsPlugin.log(
          Level.INFO,
          String.format(
              "Enchantment '%s' does not exist in this version.", enchantmentOption.getName()));
      return null;
    }

    // Increase enchantment level if necessary and allowed
    while (getValue(enchantmentValue, (enchantmentLevel + 1) * amount) < maxValue
        && enchantmentLevel < maxEnchantmentLevel) {
      enchantmentLevel++;
    }
    ...
    // Increase amount if necessary and allowed
    double maxAmountFactor = Config.getQuantityFactor() * Config.getRewardFactor() * 0.75;
    while (getValue(enchantmentValue, enchantmentLevel * (amount + 1)) < maxValue
        && amount < enchantmentOption.getMax() * maxAmountFactor) {
    ...
    if (!(itemMeta instanceof EnchantmentStorageMeta)) {
      BasicQuestsPlugin.log(
          Level.SEVERE,
          "Could not find EnchantmentStorageMeta for item with Material: " + item.getType().name());
      return null;
    }
  ...
  private static RewardItem getPotionReward(GenerationOption potionOption, double maxValue) {
    ...
    variant =
        (String)
            potionOption
                .getVariants()
                .keySet()
                .toArray()[r.nextInt(potionOption.getVariants().size())];
    if (variant.equalsIgnoreCase("EX")) extended = true;
    else if (variant.equalsIgnoreCase("UP")) upgraded = true;
    ...
    double maxAmountFactor = Config.getQuantityFactor() * Config.getRewardFactor() * 0.75;
    while (getValue(materialValue, amount + 1) < maxValue
        && amount < potionOption.getMax() * maxAmountFactor) {
  ...
  private static double getValue(
      double materialValue, int amount, double enchantmentValue, int enchantmentLevel) {
