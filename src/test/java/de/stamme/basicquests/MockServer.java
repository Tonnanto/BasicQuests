...
public class MockServer {
  ...
  public static void init() {
    ...
    when(server.getPlayer(anyString()))
        .thenAnswer(
            invocation -> {
              QuestPlayer questPlayer =
                  BasicQuestsPlugin.getPlugin()
                      .getQuestPlayer(
                          UUID.fromString(String.valueOf(invocation.getArguments()[0])));
              if (questPlayer != null) return questPlayer.getPlayer();
              else return null;
            });
    ...
    when(basicQuestsPlugin.getQuestPlayer(any(Player.class)))
        .thenAnswer(
            invocation -> {
    ...
    when(basicQuestsPlugin.getQuestPlayer(any(UUID.class)))
        .thenAnswer(
            invocation -> {
              Object[] args = invocation.getArguments();
