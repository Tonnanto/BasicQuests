...
public class QuestGeneratorTest {
  ...
  private static final Map<QuestType, Map<GenerationOption, Integer>> optionCounterForQuestType =
      new HashMap<>();
  ...
  public String getMaterialNameForQuest(Quest quest) {
    switch (quest.getQuestType()) {
      ...
      case CHOP_WOOD:
        ...
        if (chopWoodQuest.getMaterialString() != null
            && !chopWoodQuest.getMaterialString().isEmpty())
          return chopWoodQuest.getMaterialString();
        else return ((ChopWoodQuest) quest).getMaterial().name();
      case VILLAGER_TRADE:
      ...
      for (Map.Entry<GenerationOption, Integer> materialEntry :
        ...
        String materialCountString =
            String.format(
                ...
                actualWeight * 100,
                materialEntry.getKey().getWeight() * 100);
        sb.append("\n- ").append(materialCountString);

        //                Assertions.assertTrue(Math.abs(materialEntry.getKey().getWeight() -
        // actualWeight) < 0.1);
      }
    }
